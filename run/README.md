...
We do not include a precompiled executable, as binaries are often system-dependent and may not run reliably across platforms; instead, we provide the full source code and build instructions to ensure reproducibility.
...
- move the compile and code folder in the `dar12/` directory: `mv  ../../build_3D/` and `mv  ../../code_vdarwin_3P1V1Z_3D/`
- compile the code: `cd build_3D/`, run `./compile.sh 1` the output file is the executable `mitgcmuv_1` used to run simulations
- move all simulation folders in the `dar12` folder: `cd ../`, `mv ../../../Simulations/*/ .`
- move the `Post_processing folder` in the `dar12` folder: `mv ../../../Post_processing/ .`
